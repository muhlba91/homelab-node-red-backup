...
class TestMain:
    def test_check_command_exit_codes_returns_zero_when_data_exists(self, monkeypatch):
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.data_exists", lambda e, j: True
        )
    ...
    def test_check_command_exit_codes_returns_one_when_no_data(self, monkeypatch):
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.data_exists", lambda e, j: False
        )
        with pytest.raises(SystemExit) as exc:
            main_mod.check.callback("http://x", None)
    ...
    def test_backup_writes_file(self, monkeypatch, tmp_path):
        fake_backup = {"flows": []}
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.create_backup", lambda e, j: fake_backup
        )
    ...
    def test_restore_reads_file_and_calls_restore(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.restore_backup", fake_restore
        )
    ...
    def test_auto_calls_create_backup_when_data_exists(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.data_exists", lambda e, j, f=flows: True
        )
    ...
    def test_auto_calls_restore_when_no_data(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "homelab_node_red_backup.__main__.data_exists", lambda e, j, f=flows: False
        )
